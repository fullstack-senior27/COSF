...
import '../helpers/base_screen_view.dart';
import '../helpers/base_view_model.dart';

final filePickerProvider = ChangeNotifierProvider((ref) => FilePickerProvider(ref: ref));

class FilePickerProvider extends BaseViewModel<BaseScreenView> {
  Ref ref;
  FilePickerProvider({required this.ref}) : super();
  ...
  bool _imgLoader = false;
  bool get imgLoader => _imgLoader;
  set imgLoader(bool value) => imgLoader = value;
                  ...
                  MaterialButton(
                    ...
                    onPressed: () async {
                      if (kIsWeb) {
                        ...
                        if (image != null) {
                          _imgUrl = "";
                          Logger.printSuccess("FORMAT =====> ${image.fileName}");
                          _imgUrl = await uploadFileWeb(image.data, image.fileName?.split(".").last ?? "");
                          Logger.printWarning("imgUrl $_imgUrl");
                        ...
                        await pickImage(ImageSource.gallery).then((image) async {
                          if (image == null) {
                            ...
                            toggleLoading();
                            file = image;
                            Logger.printWarning("file $file");
                            setImg("");

                            setImg(await uploadFile(file!));
                            // file = image;
                            Logger.printWarning("imgUrl $imgUrl");
                            toggleLoading();
                            notifyListeners();
                          }
                        });
  ...
  Future<String> uploadFileWeb(Uint8List? bytes, String extension) async {
    toggleLoading();
    final request = http.MultipartRequest(
      'POST',
      Uri.parse("${AppConstants.baseUrl}file-upload/"),
    ...
    String name = '${generateRandomAlphanumeric(10)}.$extension';
    final webFile = html.File([blob], name, {'type': blob.type});
    ...
    if (response.statusCode == 200) {
      final responseBody = await response.stream.bytesToString();
      Logger.printSuccess(responseBody);
